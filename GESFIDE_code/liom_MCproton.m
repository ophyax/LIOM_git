% This is a prototype for ProtonMC that will go from a pO2 graph
...
function liom_MCproton
% (B0,phi_angle,omega_angle,TE,Gx,ROIx1,ROIx2,...
%     ROIy1,ROIy2,ROIz1,ROIz2,file_name,iFrame,volume_and_mesh_file,...
%     vol_iFrame,ref_file,ref_iFrame,vol_ref_iFrame)
try
%Selection of data
...
%Argument list from launcher:
% $Bfield $phi_angle ${omega_angle_list[i]} $TE $Gx $ROIx1 $ROIx2 
% $ROIy1 $ROIy2 $ROIz1 $ROIz2 $O2idx $Flowidx $O2baseidx $Flowbaseidx)

%Sample Inputs -- for mouse_20100203
% Bfield=3;
% 
% #Echo time
% TE=30; #in msec
...
save_name=['MCproton_' save_name_str sprintf(...
    ...
    B0,phi_angle,omega_angle,TE,Gx,ROIx1,ROIx2,ROIy1,ROIy2,ROIz1,ROIz2,iFrame,vol_iFrame) '.mat'];
...
SatO2mask_S = zeros(ny,nx,nz);

Hctmask_ini = zeros(ny,nx,nz);
Hctmask = zeros(ny,nx,nz);
Hctmask_S = zeros(ny,nx,nz);
                    ...
                    for iY = -ry:+ry
                        for iZ = -rz:+rz
                            if norm([iX*Hvox iY*Hvox iZ*Hvox])<=r %all in um
                                ...
                                if im2.segVesType(im2.edgeSegN(iiE))==1 %arteries
                                    SatO2mask_S(iiy,iix,iiz) = SatO2_value;
                                    Hctmask_S(iiy,iix,iiz) = Hct(im2.segVesType(im2.edgeSegN(iiE)));
                                    Vm_S(iiy,iix,iiz) = 1;
                                end
                    ...
                    for iY = -ry:+ry
                        for iZ = -rz:+rz
                            if norm([iX*Hvox iY*Hvox iZ*Hvox])<=r %all in um
                                ...
                                if im2.segVesType(im2.edgeSegN(iiE))==2 %capillaries
                                    SatO2mask_S(iiy,iix,iiz) = SatO2_value_ref;
                                    Hctmask_S(iiy,iix,iiz) = Hct(im2.segVesType(im2.edgeSegN(iiE)));
                                    Vm_S(iiy,iix,iiz) = 1;
                                elseif im2.segVesType(im2.edgeSegN(iiE))==3 %veins
                                    SatO2mask_S(iiy,iix,iiz) = SatO2_value_ref;
                                    Hctmask_S(iiy,iix,iiz) = Hct(im2.segVesType(im2.edgeSegN(iiE)));
                                    Vm_S(iiy,iix,iiz) = 1;
                                end
...
%T2 volume (in msec)
T2mask_S = T2_tissu.*ones(ny,nx,nz);
T2mask_S(Hctmask_S~=0) = 1000.*(1./( (2.74.*B0-0.6)+(12.67.*B0^2.*(1-SatO2mask_S(Hctmask_S~=0)).^2) ));
...
T2starmask_S = T2star_tissu.*ones(ny,nx,nz);
T2starmask_S(Hctmask_S~=0) = 1000.*(1./(As+Cs.*(1-SatO2mask_S(Hctmask_S~=0)).^2));
...
dChimask_S = dChi0.*Hctmask_S.*(1-SatO2mask_S);
clear Hctmask Hctmask_ini Hctmask_S
...
dChimask_S = dChimask_S(xidx1:xidx2,yidx1:yidx2,zidx1:zidx2);
...
fft_succep_volume_S=fftshift(fftn(fftshift(dChimask_S)));
clear dChimasl dChimask_ini dChimask_S 
...
delta_B_S=real(ifftshift(ifftn(ifftshift(fft_pert_B.*fft_succep_volume_S))));
clear fft_succep_volume fft_succep_volume_ini fft_succep_volume_S
...
phase_GE_S=zeros(nprotons,1);
phase_SE_S=zeros(nprotons,1);
...
signal_GE_S=zeros(1,ntime_step);
signal_SE_S=zeros(1,ntime_step);
...
protons_pos_S=[protons_pos_x protons_pos_y protons_pos_z];    
...
for i=1:ntime_step
    ...
    %identify extravascular proton
    indices_S=ceil(protons_pos_S/Hvox(1));
    indices_S(:,1)=max(min(indices_S(:,1),nx),1);
    indices_S(:,2)=max(min(indices_S(:,2),ny),1);
    indices_S(:,3)=max(min(indices_S(:,3),nz),1);
    proton_Vm_S=Vm_S(sub2ind(size(Vm_S),indices_S(:,1),indices_S(:,2),indices_S(:,3))); 
    
    %move proton temporarly
    sigma=sqrt(2*Dvox*dt);
    rnum_S=sigma*randn(nprotons,3);
    protons_pos_S_new_temp=protons_pos_S+rnum_S;
    
    %(make sure IV stays IV and EV stays EV)
    indices_S=ceil(protons_pos_S_new_temp/Hvox(1));
    indices_S(:,1)=max(min(indices_S(:,1),nx),1);
    indices_S(:,2)=max(min(indices_S(:,2),ny),1);
    indices_S(:,3)=max(min(indices_S(:,3),nz),1);
    proton_Vm_S_new=Vm_S(sub2ind(size(Vm_S),indices_S(:,1),indices_S(:,2),indices_S(:,3))); 
    
    %find the ones who crossed vessel boundary (try 100 times otherwise let it go cross the boundary)
    crossing_list=find(proton_Vm_S~=proton_Vm_S_new);
    flag_cpt=0;
    while ( ~isempty(crossing_list) && flag_cpt<=100 )
        
        rnum_S(crossing_list,:)=sigma*randn(length(crossing_list),3);
        protons_pos_S_new_temp=protons_pos_S+rnum_S;
    ...
    %move proton accordingly
    protons_pos_S=protons_pos_S+rnum_S;
    indices_S=ceil(protons_pos_S/Hvox(1));
    indices_S(:,1)=max(min(indices_S(:,1),nx),1);
    indices_S(:,2)=max(min(indices_S(:,2),ny),1);
    indices_S(:,3)=max(min(indices_S(:,3),nz),1);
    ...
    %update IV and EV lists
    proton_Vm_S=Vm_S(sub2ind(size(Vm_S),indices_S(:,1),indices_S(:,2),indices_S(:,3)));
    IV_list_S=find(proton_Vm_S==1);
    EV_list_S=find(proton_Vm_S~=1);
    display(sprintf('%1.4f percent of protons were intravascular for specific compartment',100.*length(IV_list_S)./nprotons))
    ...
    protons_B_S=delta_B_S(sub2ind(size(delta_B_S),indices_S(:,1),indices_S(:,2),indices_S(:,3)));
    ...
    protons_T2_S=T2mask_S(sub2ind(size(delta_B_S),indices_S(:,1),indices_S(:,2),indices_S(:,3)));
    ...
    protons_T2star_S=T2starmask_S(sub2ind(size(delta_B_S),indices_S(:,1),indices_S(:,2),indices_S(:,3)));
    ...
    protons_gradient_GE_S=ge_gradient_weight*(protons_pos_S(:,1)-(nx/2)*Hvox(1))*Gxum;
    protons_gradient_SE_S=se_gradient_weight*(protons_pos_S(:,1)-(nx/2)*Hvox(1))*Gxum;
    ...
    %signal GE
    phase_GE(EV_list)=phase_GE(EV_list)+1i*gamma*(protons_B(EV_list)+protons_gradient_GE(EV_list))*dt-(1./protons_T2(EV_list))*dt;
    ...
    %signal SE
    phase_SE(EV_list)=phase_SE(EV_list)+1i*gamma*(protons_B(EV_list)+protons_gradient_SE(EV_list))*dt-(1./protons_T2(EV_list))*dt;
    ...
    %%%%%%%%%%%%%%%
    % compartment specific signal (all vessels)
    
    %signal GE
    phase_GE_S(EV_list_S)=phase_GE_S(EV_list_S)+1i*gamma*(protons_B_S(EV_list_S)+protons_gradient_GE_S(EV_list_S))*dt-(1./protons_T2_S(EV_list_S))*dt;
    phase_GE_S(IV_list_S)=phase_GE_S(IV_list_S)-(1./protons_T2star_S(IV_list_S))*dt;
    signal_GE_S(i)=abs(sum(exp(phase_GE_S))/nprotons);
    
    %signal SE
    phase_SE_S(EV_list_S)=phase_SE_S(EV_list_S)+1i*gamma*(protons_B_S(EV_list_S)+protons_gradient_SE_S(EV_list_S))*dt-(1./protons_T2_S(EV_list_S))*dt;
    phase_SE_S(IV_list_S)=phase_SE_S(IV_list_S)-(1./protons_T2_S(IV_list_S))*dt;
    signal_SE_S(i)=abs(sum(exp(phase_SE_S))/nprotons);
...
BOLD_GE_S=signal_GE_S(echo_index);
BOLD_SE_S=signal_SE_S(echo_index);

%save
eval(sprintf('save %s tMCproton TE signal_GE signal_SE signal_GE_IV signal_SE_IV signal_GE_EV signal_SE_EV      signal_GE_base signal_SE_base signal_GE_IV_base signal_SE_IV_base signal_GE_EV_base signal_SE_EV_base      signal_GE_S signal_SE_S    BOLD_GE BOLD_SE BOLD_GE_IV BOLD_SE_IV BOLD_GE_EV BOLD_SE_EV      BOLD_GE_base BOLD_SE_base BOLD_GE_IV_base BOLD_SE_IV_base BOLD_GE_EV_base BOLD_SE_EV_base    BOLD_GE_S BOLD_SE_S',save_name)) %this is a small file
%eval(sprintf('save %s tMCproton signal_GE signal_SE BOLD_GE BOLD_SE TE signal_GE_base signal_SE_base BOLD_GE_base BOLD_SE_base     signal_GE_IV_only signal_SE_IV_only BOLD_GE_IV_only BOLD_SE_IV_only      signal_GE_EV_only signal_SE_EV_only BOLD_GE_EV_only BOLD_SE_EV_only   signal_GE_IV_iso signal_SE_IV_iso BOLD_GE_IV_iso BOLD_SE_IV_iso signal_GE_EV_iso signal_SE_EV_iso BOLD_GE_EV_iso BOLD_SE_EV_iso  signal_GE_IV_iso_base signal_SE_IV_iso_base BOLD_GE_IV_iso_base BOLD_SE_IV_iso_base signal_GE_EV_iso_base signal_SE_EV_iso_base BOLD_GE_EV_iso_base BOLD_SE_EV_iso_base ',save_name)) %this is a small file
