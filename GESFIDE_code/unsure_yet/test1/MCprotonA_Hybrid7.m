...
function MCprotonA_Hybrid7(B0,phi_angle,omega_angle,TE,Gx,ROIx1,ROIx2,ROIy1,ROIy2,ROIz1,ROIz2,file_name,iFrame,volume_and_mesh_file,vol_iFrame,ref_file,ref_iFrame,vol_ref_iFrame)
...
nprotons=1e7;
dt=0.2;     %msec
ntime_step=ceil(66/dt); %#########time harcoded here for the test#########
ROIx=[ROIx1 ROIx2];    %um
ROIy=[ROIy1 ROIy2];    %um
ROIz=[ROIz1 ROIz2];    %um
...
for i=1:ntime_step
    ...
    %signal GE
    phase_GE_S(EV_list_S)=phase_GE_S(EV_list_S)+j*gamma*(protons_B_S(EV_list_S)+protons_gradient_GE_S(EV_list_S))*dt-(1./protons_T2_S(EV_list_S))*dt;
    phase_GE_S(IV_list_S)=phase_GE_S(IV_list_S)-(1./protons_T2star_S(IV_list_S))*dt;
    signal_GE_S(i)=abs(sum(exp(phase_GE_S))/nprotons);
    
    %signal SE
    phase_SE_S(EV_list_S)=phase_SE_S(EV_list_S)+j*gamma*(protons_B_S(EV_list_S)+protons_gradient_SE_S(EV_list_S))*dt-(1./protons_T2_S(EV_list_S))*dt;
