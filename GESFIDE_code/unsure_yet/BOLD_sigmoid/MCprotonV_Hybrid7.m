...
function MCprotonV_Hybrid7(B0,phi_angle,omega_angle,TE,Gx,ROIx1,ROIx2,ROIy1,ROIy2,ROIz1,ROIz2,file_name,iFrame,volume_and_mesh_file,vol_iFrame,ref_file,ref_iFrame,vol_ref_iFrame)
...
save_name=['MCprotonV_Hybrid7_' save_name_str sprintf('_B0_%2.1fT_ori_[%d_%d]_TE%dms_Gx%d_ROI_[%d %d %d %d %d %d]_frame_%d_volFrame_%d',B0,phi_angle,omega_angle,TE,Gx,ROIx1,ROIx2,ROIy1,ROIy2,ROIz1,ROIz2,iFrame,vol_iFrame) '.mat'];
                    ...
                    for iY = -ry:+ry
                        for iZ = -rz:+rz
                            if norm([iX*Hvox iY*Hvox iZ*Hvox])<=r %all in um
                                ...
                                if im2.segVesType(im2.edgeSegN(iiE))==3 %veins
                        ...
                        for iZ = -rz:+rz
                            if norm([iX*Hvox iY*Hvox iZ*Hvox])<=r %all in um
                                ...
                                if im2.segVesType(im2.edgeSegN(iiE))==1 %arteries
                                    ...
                                    Hctmask_S(iiy,iix,iiz) = Hct(im2.segVesType(im2.edgeSegN(iiE)));
                                    Vm_S(iiy,iix,iiz) = 1;
                                elseif im2.segVesType(im2.edgeSegN(iiE))==2 %capillaries
                                    SatO2mask_S(iiy,iix,iiz) = SatO2_value_ref;
                                    Hctmask_S(iiy,iix,iiz) = Hct(im2.segVesType(im2.edgeSegN(iiE)));
                                    Vm_S(iiy,iix,iiz) = 1;
                                end
